...
class ModifiedColorizer(BaseColor):
    def __init__(self, config):
        ...
        self.model1 = build_basic_block(channels=[1, 64, 64], kernel_size=3, stride=[1, 2], dropout=dropoutLayers[0])
        self.model2 = build_basic_block(channels=[64, 128, 128], kernel_size=3, stride=[1, 2], dropout=dropoutLayers[1])
        self.model3 = build_basic_block(channels=[128, 256, 256, 256], kernel_size=3, stride=[1, 1, 2], dropout=dropoutLayers[2])
        self.model4 = build_basic_block(channels=[256, 512, 512, 512], kernel_size=3, dropout=dropoutLayers[3])
        self.model5 = build_basic_block(channels=[512, 512, 512, 512], kernel_size=3, dilation=2, padding=2, dropout=dropoutLayers[4])
        self.model6 = build_basic_block(channels=[512, 512, 512, 512], kernel_size=3, dilation=2, padding=2, dropout=dropoutLayers[5])
        self.model7 = build_basic_block(channels=[512, 512, 512, 512], kernel_size=3, dropout=dropoutLayers[6])

        additional = []
        for _ in range(numExtraConv2DLayers):
              newLayer = build_basic_block(channels=[512, 512, 512, 512], kernel_size=3)
...
<<<<<<< HEAD
        # TODO(Rudy) - need to figure out how to take additinal layers and incorporate them here
